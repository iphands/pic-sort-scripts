...
class NoValidDate(Exception):
    pass


class UnsupportedSuffix(Exception):
    pass
...
def try_datetime(img, key):
    if key not in img and not key in dir(img):
        return
    dt = img[key].split(":")
    data = {
        "year": dt[0],
...
def try_from_stat(img):
    ...
    for i in dates:
        if i > min_date:
            ...
            break
    if date == None:
        raise NoValidDate(f"Cant find valid date in {img}: {dates}")
...
def _get_data_norm(f, image_file):
    ...
    if not data:
        data = try_datetime(my_image, "datetime_original")
    if not data:
        data = try_datetime(my_image, "datetime")
    if not data:
        data = try_from_stat(f)
    if not data:
        raise NoDateTime()
...
def dofile(f):
    ...
    with open(f, "rb") as image_file:
        ...
        if not data:
            raise Exception(
                f"BUG: Matched suffix, ran get_data but filed data == None: {f})"
            )
        ...
        md5sum = md5(f)
        newpath = (
            f'{directory}/{data["hour"]}{data["min"]}{data["sec"]}-{md5sum}.{suff}'
        )
